    ...
    try:
        ...
        if path:
            ...
            if (path == f"/{HANDLER}") and (http_method == 'GET'):
                ...
                if not validate_input(query_string_parameters, REQUIRED_QUERY_PARAMS):
                    raise Exception("Invalid User Input - missing required field or contains extra field.")
                
                response = asyncio.run(get_user_data(query_string_parameters))
                log.info("Sleeper use found and loaded.")
